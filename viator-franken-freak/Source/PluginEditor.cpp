...
, _filterComp(audioProcessor)
, _fxComp(audioProcessor)
, _arpComp(audioProcessor)
{
    // header
    ...
    // fx comp
    addAndMakeVisible(_fxComp);
    
    // arp
    addAndMakeVisible(_arpComp);
...
void ViatorfrankenfreakAudioProcessorEditor::resized()
{
    ...
    _fxComp.setBounds(osc1AreaX, effectsY, osc1AreaWidth * effectsWidthMult, osc1AreaHeight * effectsHeightMult);
    ...
    _adsrComp.setBounds(adsrX, osc1AreaY, osc1AreaWidth, osc1AreaHeight * adsrHeightMult);
